...
impl Provisioning {
    ...
    pub fn generate_code(&self) -> Result<String> {
        ...
        tracing::trace!(trace_id, task = "generate_code", "init",);
    ...
    async fn sign_csr(
        ...
        let url = format!("{}{}", &self.settings.server_url, cert_signing_url);
    ...
    /**! This function performs the device provisioning, the flow works as follows -
    ...
    */
    pub async fn provision_me(&self, code: &str) -> Result<bool> {
        let trace_id = find_current_trace_id();
        ...
        let private_key_status = match manifest.cert_key_pair_algorithm {
            PrivateKeyAlgorithm::ECDSA => generate_ec_private_key(
                &self.settings.paths.device.private_key,
        ...
        match self.write_certificates_to_path(
